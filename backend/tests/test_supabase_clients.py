...
def _fresh_clients(monkeypatch):
    ...
    monkeypatch.setattr(db.settings, "supabase_url", "https://project.supabase.co")
...
def test_data_client_uses_service_role_key():
    with patch("app.db.supabase.create_client") as create_client:
        db.get_client()
    create_client.assert_called_once_with("https://project.supabase.co", "service-role-key")


def test_auth_client_uses_anon_key():
    with patch("app.db.supabase.create_client") as create_client:
        db.get_auth_client()
    create_client.assert_called_once_with("https://project.supabase.co", "anon-key")


def test_clients_are_cached_and_kept_separate():
    with patch("app.db.supabase.create_client", side_effect=[MagicMock(name="service"), MagicMock(name="anon")]) as create_client:
...
def test_data_access_never_uses_the_auth_client():
    ...
    with patch("app.db.supabase.get_client", return_value=service), \
